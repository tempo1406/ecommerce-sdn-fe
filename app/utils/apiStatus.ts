...
export async function checkApiAvailability(): Promise<{available: boolean, message: string}> {
  ...
  const apiUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:3000/api';
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        ...
        message = 'No response received from API server. Check if server is running.';
      } else {
      ...
      if (error.message.includes('Network Error')) {
        message = 'Network Error: Possible causes include CORS issues, server not running, or incorrect API URL. Check that your backend server is running and has proper CORS configuration.';
      }
    } else {
