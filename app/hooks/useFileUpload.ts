...
export function useFileUpload({
  ...
  const uploadToBackend = useCallback(async (file: File): Promise<string> => {
    ...
    reader.onload = (e) => {
      ...
      console.log('🖼️ FileReader onload - SUCCESS!');
      console.log('🖼️ DataURL length:', dataUrl?.length);
      console.log('🖼️ DataURL start:', dataUrl?.substring(0, 100));
      console.log('🖼️ File type from DataURL:', dataUrl?.split(';')[0]);
      
      // Validate the data URL format
      if (dataUrl && dataUrl.startsWith('data:image/')) {
        setPreview(dataUrl);
        console.log('✅ Preview state updated with valid dataURL');
        
        // Call onUpload with the preview data immediately for form handling
        if (onUpload) {
          console.log('🖼️ Calling onUpload callback with dataURL');
    ...
    reader.onloadstart = () => {
      console.log('🖼️ FileReader started reading file');
    };
    
    reader.onprogress = (e) => {
      console.log('🖼️ FileReader progress:', e.loaded, '/', e.total);
    };
    
    console.log('🖼️ About to call FileReader.readAsDataURL()');
    reader.readAsDataURL(file);
    console.log('🖼️ FileReader.readAsDataURL() called');
  ...
  const uploadSelectedFile = useCallback(async (): Promise<string | null> => {
    ...
    try {
      if (uploadToCloudinary) {
        console.log('Uploading file to Cloudinary via backend:', selectedFile.name);
  ...
  }, [selectedFile, uploadToCloudinary, uploadToBackend]);  const clearFile = useCallback(() => {
    setPreview(null);
    setFileName(null);
