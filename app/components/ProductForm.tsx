...
export default function ProductForm({ product, onSubmit, buttonText }: ProductFormProps) {
  ...
  const handleFormSubmit = async (data: Product) => {
    try {
      ...
      if (uploadImageFunction && data.image?.startsWith('data:')) {
        console.log('Uploading image to Cloudinary before creating product...');
      ...
      if (errorMessage.includes('Network error')) {
        toast.error(
          <div>
            ...
            <p className="text-sm mt-1">Cannot connect to server. Using mock data mode.</p>
          </div>, 
          { autoClose: 5000 }
        );
      } else {
    ...
    >
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          value={watch('image')}
          onChange={(imageData) => {
            console.log('🎯 ProductForm onChange called:', imageData ? imageData.substring(0, 50) + '...' : 'null');
            setValue('image', imageData);
            console.log('🎯 setValue called with imageData');
            console.log('🎯 Current form image value:', watch('image')?.substring(0, 50));
          }}
          disabled={isSubmitting}
          uploadToCloudinary={true}
          onGetUploadFunction={(uploadFn) => {
            console.log('🎯 Setting upload function:', uploadFn);
            setUploadImageFunction(() => uploadFn);
          }}
        /><p className="mt-1 text-xs text-gray-500">
          Upload an image file - will be uploaded to Cloudinary when you submit the form
        </p>
      </div>
